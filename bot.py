...
# Вопросы теста
questions = [
    "Как вы реагируете, если допустили небольшую ошибку в своей работе?\n\n"
    ...
    "D. Удивляюсь, почему я не смог справиться сразу.\n"
    "E. Чувствую себя некомпетентным во всех своих ролях (например, сотрудник/сотрудница, родитель, друг/подруга).",
    ...
    "Что вы чувствуете, если кто-то другой делает работу лучше вас?\n\n"
    "А. Думаю, что нужно было больше времени уделить качеству своей работы.\n"
    ...
    "Что для вас означает идеальный результат?\n\n"
    ...
    "Как вы готовитесь к выполнению важной задачи?\n\n"
    "А. Проверяю каждую деталь, чтобы избежать ошибок.\n"
    "B. Изучаю всю доступную информацию, чтобы быть полностью подготовленным/ой.\n"
    "C. Планирую сделать все самостоятельно, чтобы избежать проблем.\n"
    ...
    "Как вы воспринимаете успехи других людей?\n\n"
    ...
    "Как вы ощущаете себя в разных ролях (работник, родитель, друг и т.д.)?\n\n"
...
# Описание типов
types_description = {
    "A": "Перфекционист: Вам важно, чтобы все было идеально. Небольшая ошибка может вызвать разочарование. "
         "Обратите внимание, что ошибки — это часть роста.",
    "B": "Эксперт: Вы стремитесь знать всё о своей области. Помните, что никто не может знать абсолютно всё.",
    "C": "Соло-игрок: Вы предпочитаете делать всё самостоятельно. Позвольте другим помочь вам — это не признак слабости.",
    "D": "Природный гений: Вам важно справляться быстро и легко. Поймите, что не всегда всё получается с первого раза.",
    "E": "Сверхчеловек: Вы пытаетесь быть лучшей во всех сферах жизни. Установите для себя реалистичные ожидания."
}
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_question + 1 < len(questions):
        ...
        await update.message.reply_text(
            f"Спасибо за прохождение теста! Ваш тип: {result}\n\n{types_description[result]}"
        )
